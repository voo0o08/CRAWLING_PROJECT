...
cnt = 1
max_page_num=20 # 몇번째 페이지까지 넘길건지
word_list = []
data_dict = {"모집직종":[],
             "직무내용":[],
             "경력조건":[],
...
for page_num in range(1, max_page_num+1):
    ...
    url = f"https://m.work.go.kr/wnSearch/unifSrchResult.do?keyword=AI&srchSort=10&Query=AI&tabName=tb_workinfo&menu=workInfo&pageIndex={page_num}"
    ...
    for row in soup.select("#dataList_2>li>a"):
        ...
        time.sleep(1) # sleep 안넣으면 속도차 때문에 page_source불러올 때 오류남
...
df.to_csv('work_net_AI.csv', sep=',', na_rep='NaN', encoding='utf-8')