...
text = open('gitadata.csv', encoding='utf-8').read()
okt = Okt() # Open Korean Text 객체 생성
# okt함수를 통해 읽어들인 내용의 형태소를 분석한다.
sentences_tag = []
sentences_tag = okt.pos(text.upper())
noun_adj_list = []
...
tag_dict = dict(tags)
print(tag_dict)
stopwords = ['/','"',"'", '기타','건','for','X','FOR','삼성','국내','참여','활용','고급','개월']
...
background_color="white", max_font_size=100,# 제일 많이 나온 단어의 사이즈
